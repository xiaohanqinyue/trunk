...
public class UserController {
    ...
    public String loginValidate(HttpServletRequest request, HttpServletResponse response,User user) {
        ...
        if(cur_user != null) {
            String pwd = MD5.md5(user.getPassword()); //对填写的用户密码进行md5加密
    ...
    public ModelAndView changePassword(HttpServletRequest request,User user,ModelMap modelMap) {
        String url=request.getHeader("Referer");
        //从session中获取出当前用户
        User cur_user = (User)request.getSession().getAttribute("cur_user");
        cur_user.setPassword(MD5.md5(user.getPassword()));//更改当前用户的用户名
        userService.updateUserName(cur_user);//执行修改操作
        return new ModelAndView("redirect:"+url);
    }
    /**
    ...
    public ModelAndView goods(HttpServletRequest request) {
        ...
        //将相应的商品信息、图片信息返回到前台显示
        mv.addObject("goodsAndImage",goodsAndImage);
        mv.setViewName("/user/goods");  //指定视图名称
        return mv;
    }
